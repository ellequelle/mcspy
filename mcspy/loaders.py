...
def load_mix_dframe(year):
    ...
    '''
    fn = MCS_DATA_PATH + f'DATA/{year}/{year}_mixvars'
    fname = addext(fn, '.npy')
    # load the numeric data and make a new DataFrame
    mix = np.load(fname, mmap_mode='c')
...
def load_mix_var(year, varname):
    '''Reads a metadata index variable for `varname` from `year` as a numpy array from the numpy array file "{year}/{year}_{varname}_index.npy".'''
...
def load_prof_var(year, varname):
    '''Reads the profile data variable `varname` from `year` from the numpy array file "{year}/{year}_{varname}_profiles.npy" and returns a single 2-D array. If `varname` == "pressure", the returned array is shape (105,1).'''
    # handle pressure separately
    if varname == 'pressure' or 'varname' == 'prs':
        return 610*np.exp(-0.125*(np.arange(105)-9)).reshape((105,1))
    fname = MCS_DATA_PATH + f'DATA/{year}/{year}_{varname}_profiles.npy'
    # load data
    with gzip.open(fname, 'rb') as fout:
        var = np.load(fout).reshape((-1,105)).transpose()
    print(f'loaded {fname}')
    return var
...
def load_prof_var_years(years=[2006,2007,2008,2009,2010],
    ...
    return np.concatenate(dat, axis=1)
