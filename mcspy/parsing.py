...
def load_tab_file(prodid, dfindex, **kwargs):
    '''Load the data from the raw text file. Downloads the file if it isn't found locally.
    Returns a DataFrame with metadata and a DataFrame with the actual profile data.'''
...
def parse_tab_file(fn, meta=True, data=True,
                    mix_keep_cols=mix_keep_cols):
    '''Given a TAB file name, parse the MCS file and return the metadata (location, Ls, LST, etc) and the profiles in two DataFrames.
    meta: whether to return the metadata DataFrame
    data: whether to return the profile data DataFrame
    mix_keep_cols: columns to keep in the metadata (default value is set above)
    '''
    ...
    if data:
        ...
        dats = pd.read_csv(io.StringIO(dats), header=None, dtype=float,
...
def _read_lists(fn):
    '''_read_lists is a function to help parse_mcs_file by reading the actual TAB file, separating the metadata rows from the profile data rows, and returning both sets of strings as lists
    returns
    ...
    dats = [lines[n*106+1:(n+1)*106] for n in range(len(lines)//106)]
    return mdlines, dats
